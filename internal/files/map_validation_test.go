...
func TestValidateInstalledLocalMapData(t *testing.T) {
	tests := []struct {
		name        string
		...
		},
		{
			...
			setup: func(t *testing.T, mapRoot, cityCode string) {
				writeInstalledLocalMapFixture(t, mapRoot, cityCode, "AAA")
				roadsPath := paths.JoinLocalPath(mapRoot, cityCode, mapRoadsArchivedFileName)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config, errType, err := ValidateInstalledLocalMapData(mapRoot, cityCode)
			require.Equal(t, tt.wantErrType, errType)
			if tt.wantErr {
				require.Error(t, err)
				return
...
func TestValidateInstalledDownloadedMapData(t *testing.T) {
	tests := []struct {
		name        string
		setup       func(t *testing.T, mapRoot, cityCode string)
		wantErrType types.DownloaderErrorType
		...
		},
		{
			...
			setup: func(t *testing.T, mapRoot, cityCode string) {
				writeInstalledDownloadedMapFixture(t, mapRoot, cityCode)
				demandPath := paths.JoinLocalPath(mapRoot, cityCode, mapDemandArchivedFileName)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			errType, err := ValidateInstalledDownloadedMapData(mapRoot, cityCode)
			require.Equal(t, tt.wantErrType, errType)
			if tt.wantErr {
				require.Error(t, err)
				return
...
func writeInstalledDownloadedMapFixture(t *testing.T, mapRoot, cityCode string) {
	...
	require.NoError(t, os.WriteFile(paths.JoinLocalPath(cityPath, mapDemandArchivedFileName), []byte("{}"), 0o644))
	require.NoError(t, os.WriteFile(paths.JoinLocalPath(cityPath, mapRoadsArchivedFileName), []byte("{}"), 0o644))
	require.NoError(t, os.WriteFile(paths.JoinLocalPath(cityPath, mapRunwaysArchivedFileName), []byte("{}"), 0o644))
	require.NoError(t, os.WriteFile(paths.JoinLocalPath(cityPath, mapBuildingsArchivedFileName), []byte("{}"), 0o644))
}
