...
// FindMapCodeConflict resolves whether the provided map city code would collide
// with an already-installed (local/remote) or vanilla map.
...
func (d *Downloader) resolveMapInstallConflict(
	...
	warnAttrs := append([]any{}, baseAttrs...)
...
func (d *Downloader) importMapNow(zipPath string, replaceOnConflict bool) types.AssetInstallResponse {
	...
	mapID := types.LocalMapAssetID(configData.Code)
	version := strings.TrimSpace(configData.Version)
	if version == "" {
		version = "0.0.0"
	}

	// For local imports, detect conflicts even when the existing installed asset has the same
	// local asset ID (same city code) so that local->local replacements use the standard
	// warn-first override flow.
...
func (d *Downloader) installMapNow(ctx context.Context, mapId string, version string, replaceOnConflict bool) types.AssetInstallResponse {
	...
	inspectedConfig, configErrType, configErr := files.ValidateMapArchive(downloadResp.Path)
	if configErr != nil {
		os.Remove(downloadResp.Path)
