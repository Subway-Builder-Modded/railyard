	...
	for key, fileStruct := range filesFound {
		...
		go func(key string, fileStruct types.FileFoundStruct) {
			...
			case files.MapArchiveKeyConfig:
				destinationPath = paths.JoinLocalPath(destFolder, "config.json")
				shouldArchive = false
			case files.MapArchiveKeyTiles:
				destinationPath = paths.JoinLocalPath(d.getMapTilePath(), configData.Code+".pmtiles")
				shouldArchive = false
			case files.MapArchiveKeyThumbnail:
				destinationPath = paths.JoinLocalPath(d.getMapThumbnailPath(), configData.Code+".svg")
				shouldArchive = false
			}
