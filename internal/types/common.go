...
var mapCodePattern = regexp.MustCompile(`^[A-Z]{2,4}$`)
...
func LocalMapAssetID(cityCode string) string {
	return cityCode
}

func LocalMapCodeFromAssetID(assetID string) (string, bool) {
	if !IsValidMapCode(assetID) {
		return "", false
	}
	return assetID, true
}

func IsValidMapCode(code string) bool {
	return mapCodePattern.MatchString(code)
}
